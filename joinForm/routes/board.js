    ...
    connection.query(sql, [name], function(err, row){
      ...
      else//해당되는 제품이 존재할 경우
      {
       ...
       //그 제품 창 띄워줘야함..!
        ...
        res.render('single',{user_id : sess, admin : ok_admin, name : row[0].name, price : row[0].price, img_link1 : row[0].img_link1, img_link2 : row[0].img_link2});
  ...
  pool.getConnection(function(err, connection)
  {
    ...
    connection.query(sql, [name], function(err, row){
      ...
      else//해당되는 제품이 존재할 경우
      {
       ...
       //그 제품 창 띄워줘야함..!
        //res.redirect('/board/' + row[0].name);
        res.render('p_single',{user_id : sess, admin : ok_admin, name : row[0].name, price : row[0].price, img_link1 : row[0].img_link1, img_link2 : row[0].img_link2});
        //res.redirect('/board/single')
      }
      connection.release();
    });
